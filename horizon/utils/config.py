...
class ConfigParser:
    ...
    def __init__(self, arg):
        ...
        self.wandb_run = wandb.init(project=arg.name, config=arg.configs)
        _config = wandb.config.as_dict()
        self.config = _update_config(_config)

        wandb.run.name = self.config['dataset']['name'] + \
            '_' + self.config['arch']['name'] + \
            '_' + datetime.now().strftime(r'%m%d_%H%M%S')
