...
## Folder Structure
  ...
  │   └── config-defaults.yaml - holds configuration for training
  │
...
2. **Implementing abstract methods**
    ...
    You need to implement `_train_epoch()` for your training process, if you need validation and test then you can implement `_valid_epoch()` and `_test_model()` with provided `valid_dataloader` and `test_dataloader`.
